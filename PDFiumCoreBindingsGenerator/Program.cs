...
namespace PDFiumCoreBindingsGenerator
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var pdfiumProjectDir = Path.GetFullPath(Path.Combine(solutionDir, "PDFiumCoreBindingsGenerator", "bin"));
        ...
        private static void MyOptimize(string destinationCsPath)
        {
            var txt = File.ReadAllText(destinationCsPath);
            var result = txt.Replace("\"Setting.DllName.dll\"", "Setting.DllName");
            File.WriteAllText(destinationCsPath, result);
        }
